...
def train(train_loader, val_loader, model, criterion, optimizer, epoch, params):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(stream, start=1):
            ...
            if i % 10 == 9:  # Every 10 mini-batches...
                ...
                avg_vloss = running_vloss / len(stream)
                # Log the running loss averaged per batch
...
def train_and_validate(model, train_dataset, val_dataset, params):
    ...
    for epoch in range(1, params["epochs"] + 1):
        train(train_loader, val_loader, model, criterion, optimizer, epoch, params)
...
dataset_directory = '/Volumes/LeoSegmtSSD/CoralWORMS/TRAINING_DATA'
root_directory = os.path.join(dataset_directory)
images_directory = os.path.join(root_directory, "PATCHES/RAW_AUG_CROPPED")
masks_directory = os.path.join(root_directory, "PATCHES/LABELS_AUG_CROPPED")
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter('/Volumes/LeoSegmtSSD/CoralWORMS/MODELS/runs/')
    model = train_and_validate(model, train_dataset, val_dataset, params)
    model_save_dir ='/Volumes/LeoSegmtSSD/CoralWORMS/MODELS'
    ...
    # model2.load_state_dict(torch.load('/Users/leonardobertini/Desktop/AlienCoralEnv/WormTubeDataset/MODEL/model.pth'))
    #
    ...
    # pred_dir = os.path.join(dataset_directory,'PREDICTIONS2')
    # os.makedirs(pred_dir, exist_ok=True)
    #
    # for indx in range(0, len(predictions)):
    #     original_name = test_dataset.images_filenames[indx]
    #     original_img = cv2.imread(os.path.join(test_dataset.images_directory,test_dataset.images_filenames[indx]),cv2.IMREAD_GRAYSCALE)
    #     pred_data = predictions[indx][0]
    #     uint_img = np.array(pred_data * 255).astype('uint8')
    #     pred_name = original_name.split('.tif')[0]+'_PRED.png'
    #
    #     if np.max(pred_data) > 0:    pred_dir = os.path.join(dataset_directory,'PREDICTIONS2')
    # os.makedirs(pred_dir, exist_ok=True)
    #
    # for indx in range(0, len(predictions)):
    #     original_name = test_dataset.images_filenames[indx]
